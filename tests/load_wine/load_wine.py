...
def test_load_wine_dataset(outlier_detection_OCPC: bool):
    data = load_wine()
    erro_proposto = 0.1
    ...
    Y_adjusted_pc, metrics_pc = run_label_correction(
        ...
        f"PC_load_wine_{'OCPC' if outlier_detection_OCPC else 'LOF'}"
    )
    ...
    cl_issues = run_confident_learning(
        ...
        data_with_error["target"],
        data.target,
        f"CL_load_wine_{'OCPC' if outlier_detection_OCPC else 'LOF'}"
    )
...
if __name__ == "__main__":
    test_load_wine_datset(outlier_detection_OCPC=True)
    test_load_wine_datset(outlier_detection_OCPC=False)